...
# For pretrained_weights, type "bert_model" or "bert-base-uncased"
...
NUM_EPOCHS = 5
threshold_grid = np.linspace(0.45,0.7,26)


# train_list = [  train_toxic_loader, train_isAbuse_loader]
# val_list = [  val_toxic_loader, val_isAbuse_loader]
# name_list = [  "toxic", "IsAbuse"]
# all_acc = []
all_f1 = []
# all_roc_auc = []
all_loss = []
...
for i in tqdm.tqdm(range(len(threshold_grid))):
    ...
    for epoch in tqdm.tqdm(range(NUM_EPOCHS)):
        ...
        for j, (data_batch, batch_labels) in enumerate(train_toxic_loader):
            """
        ...
        f1 = evaluate(model, val_toxic_loader, threshold)
...
print("Current Label: Toxic")
    # print("acc: ", all_acc[i])
print("F1: ", all_f1)
    # print("ROC AUC: ", all_roc_auc[i])
for i in range(len(threshold_grid)):
    print("Loss History with threshold "+str(threshold_grid[i])+":", all_loss[i][-10:])

avg = []
for i in range(len(threshold_grid)):
    avg.append(np.mean(all_f1[i]))

max_ind = np.argmax(avg)
best_threshold = threshold_grid[max_ind]
